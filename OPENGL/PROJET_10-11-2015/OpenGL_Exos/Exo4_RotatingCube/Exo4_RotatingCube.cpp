...
size_t iPosSize = NULL;
size_t iNormSize = NULL;
...
{	
	...
	size_t iNbElement = positions.size() + normals.size();
	iPosSize = positions.size();
	iNormSize = normals.size();

	glGenBuffers(1, &g_Cube.VBO);
	glBindBuffer(GL_ARRAY_BUFFER, g_Cube.VBO);
	glBufferData(GL_ARRAY_BUFFER, (sizeof(float) * iNbElement), NULL, GL_STATIC_DRAW);

	float* fbuff = static_cast<float*>(glMapBuffer(GL_ARRAY_BUFFER, GL_WRITE_ONLY));

	for (size_t i = 0; i < iPosSize; ++i)
		fbuff[i] = positions[i];
	
	for (size_t i = 0; i < iNormSize; ++i)
		fbuff[iPosSize + i] = normals[i];
